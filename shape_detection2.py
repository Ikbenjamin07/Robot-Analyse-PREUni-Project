        ...
        for i in range(len(contour)):
            ...
            if a2 <= a1 and a2 >= a1 - 180 and abs(a1 - a2) != 0:
                ...
                if circle_length >= 50:
                    ...
                    #circulaire_contours.append(calculate_contour(contour, i, circle_length))
        if len(list_curves) > 0:
            circulaire_contours.append(contour)
    #circulaire_contours = [np.array(contour, dtype=np.int32) for contour in circulaire_contours]
...
def calculate_contour(contour, i, length):
    shape_contour = []
    for j in range(length):
        shape_contour.append(contour[(i+j) % len(contour)][0])
